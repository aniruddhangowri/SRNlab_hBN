...
def init_comm():
    ...
    #chan_d['tvc']['port'] = serial.Serial('/dev/ttyUSB1', timeout=chan_d['tvc']['timeout'])
...
def _cmd_io(cmdstr, eps=50):
    ...
    with chan_d['tvc']['lock']:
        #REMchan_d['tvc']['port'].write(_cmd_mod+cmdstr+'\n')
        if eps==0: return
        else: res = '0.0' #REM chan_d['tvc']['port'].read(eps)[:-1]
...
class tvc():
    ...
    def set_setp_state(self, args):
        ...
        #REM
        return 'OK', [setpname, self.setps[setname]]
    ...
    def activate_setp(self, args):
        #REM resp = _activate_setp(args[0])
        # check for errors!
